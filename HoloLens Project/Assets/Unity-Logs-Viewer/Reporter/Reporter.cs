...
public class Reporter : MonoBehaviour
{
	...
	void DrawInfo()
	{
		...
		GUILayout.Space(size.x);
		GUILayout.Space(size.x);
		GUILayout.Label("Screen Width " + UnityEngine.Screen.width, nonStyle, GUILayout.Height(size.y));
		GUILayout.Space(size.x);
		GUILayout.Label("Screen Height " + UnityEngine.Screen.height, nonStyle, GUILayout.Height(size.y));
		GUILayout.FlexibleSpace();
		GUILayout.EndHorizontal();
		...
		GUILayout.Space(size.x);
		float _size = GUILayout.HorizontalSlider(size.x, 16, 64, sliderBackStyle, sliderThumbStyle, GUILayout.Width(UnityEngine.Screen.width * 0.5f));
		if (size.x != _size) {
			size.x = size.y = _size;
			initializeStyle();
		}
	...
	void DrawReport()
	{
		screenRect.x = 0f;
		screenRect.y = 0f;
		screenRect.width = UnityEngine.Screen.width;
		screenRect.height = UnityEngine.Screen.height;
		GUILayout.BeginArea(screenRect, backStyle);
	...
	void drawToolBar()
	{
		...
		toolBarRect.x = 0f;
		toolBarRect.y = 0f;
		toolBarRect.width = UnityEngine.Screen.width;
		...
		GUI.skin = toolbarScrollerSkin;
		Vector2 drag = getDrag();
		if ((drag.x != 0) && (downPos != Vector2.zero) && (downPos.y > UnityEngine.Screen.height - size.y * 2f)) {
	...
	void DrawLogs()
	{
		...
		if (drag.y != 0 && logsRect.Contains(new Vector2(downPos.x, UnityEngine.Screen.height - downPos.y))) {
		...
		int totalVisibleCount = (int)(UnityEngine.Screen.height * 0.75f / size.y);
		...
		for (int i = startIndex; (startIndex + index) < endIndex; i++) {
			...
			countRect.x = UnityEngine.Screen.width - w;
		...
		buttomRect.x = 0f;
		buttomRect.y = UnityEngine.Screen.height - size.y;
		buttomRect.width = UnityEngine.Screen.width;
	...
	void drawGraph()
	{
		...
		graphRect = stackRect;
		graphRect.height = UnityEngine.Screen.height * 0.25f;//- size.y ;
		...
		Vector2 drag = getDrag();
		if (graphRect.Contains(new Vector2(downPos.x, UnityEngine.Screen.height - downPos.y))) {
			if (drag.x != 0) {
		...
		if (graphScrollerPos.x >= (samples.Count * graphSize - UnityEngine.Screen.width))
		...
		for (int i = 0; i < UnityEngine.Screen.width / graphSize; i++) {
	...
	void drawStack()
	{
		...
		if (selectedLog != null) {
			Vector2 drag = getDrag();
			if (drag.y != 0 && stackRect.Contains(new Vector2(downPos.x, UnityEngine.Screen.height - downPos.y))) {
	...
	public void OnGUIDraw()
	{
		...
		logsRect.width = UnityEngine.Screen.width;
		logsRect.height = UnityEngine.Screen.height * 0.75f - size.y * 2f;

		stackRectTopLeft.x = 0f;
		stackRect.x = 0f;
		stackRectTopLeft.y = UnityEngine.Screen.height * 0.75f;
		stackRect.y = UnityEngine.Screen.height * 0.75f;
		stackRect.width = UnityEngine.Screen.width;
		stackRect.height = UnityEngine.Screen.height * 0.25f - size.y;



		detailRect.x = 0f;
		detailRect.y = UnityEngine.Screen.height - size.y * 3;
		detailRect.width = UnityEngine.Screen.width;
	...
	bool isGestureDone()
	{
		...
		int gestureBase = (UnityEngine.Screen.width + UnityEngine.Screen.height) / 4;
