...
Shader "Personal/HolographicWindow"
{
	Properties
	{
		// Main maps.
		...
		// Dissolve Options
		_DissolveY("Current Y of the dissolve effect",  Range(0.0, 1.0)) = 0
		_DissolveSize("Size of the effect",  Range(0.0, 1.0)) = 0.01
		_DissolveColor("Main Color", Color) = (1, 1, 1, 1)
		...
		#define PROXIMITY_LIGHT_DATA_SIZE 6
					...
					// Texturing.
						...
						clip(i.uv.y - _DissolveY + _DissolveSize);
					
						//if (i.uv.y - _DissolveY < _DissolveSize)
						//	output = _DissolveColor;

						/*
						if ((0.5f - _DissolveY < i.uv.x && i.uv.x < 0.5f + _DissolveY) && (0.5f - _DissolveY < i.uv.y && i.uv.y < 0.5f + _DissolveY))
							clip(-1);
							*/
				...
				FallBack "VertexLit"
				CustomEditor "HolographicWindowInspector"
}
